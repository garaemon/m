...
function computeCodeBlockMetaHTML(code, lang, errors) {
  if (!!errors) {
    // Only one error can be shown
    const error = errors[0];
    return `<code-meta-info language="${lang}" error-string="${error}">${code}</code-meta-info>`;
  } else {
    ...
    if (lang === undefined) {
      ...
      if (lang == 'mathjax') {
        mjAPI.typeset({
          ...
          if (data.errors) {
            const error_language_label_html = computeCodeBlockMetaHTML(
              code, `${lang}`, data.errors);
            callback(null, error_language_label_html + code);
          } else {
            callback(null, language_label_html + data.svg);
          }
        });
      } else {
        try {
